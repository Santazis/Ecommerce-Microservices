...
public class TempStorageService : ITempStorageService
{
    ...
    public TempStorageService(IAmazonS3 s3Client, IOptions<S3Settings> s3Settings, ILogger<TempStorageService> logger)
    {
        _s3Client = s3Client;
    ...
    {
        _logger.LogInformation("Saving file to temp storage");
        ...
        _logger.LogInformation("File with key {key} saved to temp storage",putRequest.Key);
